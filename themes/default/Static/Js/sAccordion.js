...
(function($) {
    $.fn.hoverAccordion = function(options) {
        ...
        var container = this;
		var this_ = $(container);
		var ULNum = this_.find('ul').length;
        // Current URL
		...
		this_.children('li>ul').hide();
            // Change only one display status at a time
            var oldList = this_.find('.' + options.classOpen).closest(
            'li').find('ul:first');
            if (false === oldList.is(':animated')) {
                if (options.keepHeight == true) {
                  	listHeight = (this_.height()-(ULNum*31))-(TT.isIE6?2:0);//maxHeight;
				}
                ...
                if (itemHeader.hasClass(options.classOpen) == false) {
					...
					//itemList.find("iframe").height(listHeight);
                    oldList.animate({
                        height : 0
                    }, {
                        step : function(n, fx) {
                        ...
                        duration : options.speed
                    }).removeClass("IsShow").children().hide();
        ...
        function() {
            ...
            if (itemList.length > 0) {
                ...
                if (options.activateItem === true) {
                    ...
                    itemHeader.removeClass(options.classClosed).addClass(
                    options.classOpen);
					var iframe = this_.find(".active iframe");
					iframe.height($("#l-m-n").height()-(ULNum*(TT.isIE6?32:31)));//
                }
            }
